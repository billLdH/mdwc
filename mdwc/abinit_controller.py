...
#***************************************
		if re.match('number_atom_fix_constrains', line):
			numb_atom_fix_cons= int(re.findall('\s*number_atom_fix_constrains\s+(\d+)', line)[0])
			#number_constrains.append(numb_bond_cons)
...
#***************************************			
			...
			numb_angl_cons= int(re.findall('\s*number_angle_constrains\s+(\d+)', line)[0])
			...
			numb_cell_para_cons= int(re.findall('\s*number_cell_parameter_constrain\s+(\d+)', line)[0])
			...
			numb_cell_angl_cons= int(re.findall('\s*number_cell_angle_constrain\s+(\d+)', line)[0])
			...
			if volu_cons != 0:
				...
				volu_cons= 1 
				bool_volu= 0
	return numb_bond_cons, bool_bond_cons, numb_angl_cons, bool_angl_cons, \
